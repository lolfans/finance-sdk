...
use FinanceSdk\Constant\SystemHeader;
use FinanceSdk\Http\HttpResponse;
...
//				}
				...
				if ($bodys) {
					curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($bodys,JSON_UNESCAPED_SLASHES));
				} elseif (0 < count($bodys)) {
					curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode(http_build_query($bodys),JSON_UNESCAPED_SLASHES));
				}
			}		
		}
